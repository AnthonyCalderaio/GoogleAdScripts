...
function main() {
  ...
  // Log the number of columns for the range named 'FILL_IN' in the
  // spreadsheet.
  var range = ss.getRangeByName('FILL_IN');
  if (range) {
  ...
  function checkSheet(category, action){
        Logger.log("Checking Sheet...")
          ...
          var sheet = ss.getSheetByName(SHEET_NAME);
             ...
             var range = sheet.getRange('A1:F26');
             ...
             for (var i = 0; i < values.length; i++) 
             {
               		...
               		if (values[i][0].toString() === thisCategory.toString() && values[i][1].toString() === thisAction.toString())
                    {
                      Logger.log("Something should happen");
                        ...
                        var range = sheet.getRange('D'+i+':F'+i);
                        range.setValues(values);
                      Logger.log("Match. Returning")
                        return;
                    }
               		//Checkfor addtocart_
               		//if(values[i][0].toString().slice(0,10) === 'addtocart_')
                    //{
...
//];
                  ...
                  //if(values[i].toString().slice(0,10) === ){
                    //Logger.log("Hereyo") 
                  //Logger.log(values[i][1])  	
                  //}
                  //Logger.log(values[i].join(',    '));
             //}
               Logger.log("Finished Checking Sheet. nothing found")
  			 }
    		//Finished checking whole shieet with no match
...
/// 	 /////                           Emails					       					                       \\\\\\\													 
    ...
    //Logger.log("Email dispatched to "+"\""+"xxxxxxxxxxxxxxxxxxxxxxx"+"\"")
  ...
  //Start logging the frequency of each button click
    var SPREADSHEET_URL = 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx';
